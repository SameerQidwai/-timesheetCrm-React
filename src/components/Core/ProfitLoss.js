...
class ProfitLoss extends Component {
  ...
  calculateProjectData = (
    ...
    let forecastStartDate = currentDate.isBetween(
      fiscalYear['start'],
      fiscalYear['end'],
      'day',
      '[]'
      ...
      ? currentDate
      : fiscalYear['start'];
    forecastStartDate = startDate.isBefore(forecastStartDate, 'day')
      ? forecastStartDate
      : startDate;
    let forecastEndDate = endDate.isAfter(fiscalYear['end'], 'day')
      ? fiscalYear['end']
      : endDate;
    //Testing
    for (
      var iDate = parseDate(startDate);
      ...
      if (
        iDate.isoWeekday() !== 6 &&
        iDate.isoWeekday() !== 7 &&
        !holidays[parseDate(iDate, true, 'M/D/YYYY')]
      ) {
        ...
        if (
          (iDate.isSameOrAfter(forecastStartDate, 'day') && // finding Fiscal Months
            iDate.isSameOrBefore(forecastEndDate),
          'day')
        ) {
          if (iDate.isSameOrAfter(parseDate(new Date()), 'month')) {
            ...
            forecast.forEach((el, index) => {
              if (
                iDate.isBetween(
                  parseDate(el.con_startDate),
                  parseDate(el.con_endDate ?? tempEndDate),
                  'day',
                  '[]'
                ) &&
                iDate.isBetween(
                  parseDate(el.res_startDate),
                  parseDate(el.res_endDate ?? tempEndDate),
                  'day',
                  '[]'
                )
              ) {
                if (forecastStatement[key]) {
                  forecastStatement[key].monthTotalBuy +=
                    el.forecastBuyRateDaily;
                  forecastStatement[key].monthTotalSell +=
                    el.forecastSellRateDaily;
                } else {
    ...
    for (
      var iMonth = parseDate(fiscalYear['start']);
      iMonth.isSameOrBefore(fiscalYear['end']);
      iMonth.add(1, 'months')
    ) {
      ...
      if (proType === 2) {
        //For timebase Project
        if (actualStatement[key]) {
          //If actual is present
        ...
        } else if (forecastStatement[key]) {
          //If forcast is found
          revenueValue =
            totalRevenue - forecastStatement[key]?.['monthTotalSell'] < 0
              ? totalRevenue ?? 0
              : forecastStatement[key]?.['monthTotalSell'] ?? 0;
          ...
          if (forecastStatement[key]?.['monthTotalSell'] > revenueValue) {
            ...
            let sellpercent =
              (revenueValue / forecastStatement[key]?.['monthTotalSell']) * 100;
            cos =
              (forecastStatement[key]?.['monthTotalBuy'] / 100) * sellpercent; //to get percentage how much cos is needed to less amount sale
          } else {
            cos = forecastStatement[key]?.['monthTotalBuy'] ?? 0; // assigning forcasting Cost
          }
        }
      } else if (proType === 1) {
        //For Project Base Project
        // revenueValue = isNaN(revenuePerDay * workDays) ? 0 : (revenuePerDay * workDays)
        revenueValue = parseFloat(segmentsRevenue[key] ?? 0);
        cos =
          actualStatement[key]?.['monthTotalBuy'] ??
          forecastStatement[key]?.['monthTotalBuy'] ??
          0;
      }
  ...
  calculateLeadData = (holidays) => {
    ...
    for (var i = 1; i <= len; i++) {
      startDate =
        i === 1 ? billing.startDate : parseDate(startDate).set('date', 1);
      endDate =
        i === len ? billing.endDate : parseDate(startDate).endOf('month');
    ...
    for (var i = 1; i <= len; i++) {
      ...
      if (
        startDate.isBetween(
          parseDate(fiscalYear['start']),
          parseDate(fiscalYear['end']),
          'month',
          '[]'
        )
      ) {
  ...
  Columns = () => {
    ...
    for (
      ...
      let color =
        parent === 'P'
          ? iMonth.isSameOrAfter(parseDate(new Date()), 'month')
            ? '#ff7875'
            : '#a0df7d'
          : '#ff7875';
  ...
  render() {
    ...
    return (
      <Row justify="start" gutter={[20, 5]}>
        <Col>
          <Tag color="#a0df7d" className="legends"></Tag>
          <span> Actual</span>
        </Col>
        <Col>
          <Tag color="#ff7875" className="legends"></Tag>
          <span> Forecast</span>
        </Col>
