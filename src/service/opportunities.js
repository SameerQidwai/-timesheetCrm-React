  ...
  return axios
    ...
    .then((res) => {
      ...
      if (success) {
        ...
        billing.goget = (
          (billing.getPercentage * billing.goPercentage) /
          100
        ).toFixed(2);
...
export const editLeadSkillResource = (crud, skillId, resId, data) => {
  ...
  console.log(crud);
  return axios
    .put(`${Api}${crud}/${skillId}/allocations/${resId}`, data, {
      headers: headers(),
    })
    .then((res) => {
      const { success, data, message } = res.data;
...
export const getCalculatedValue = (opportunityId) => {
  return axios
    .get(`${url}/${opportunityId}/calculatedValue`, { headers: headers() })
    .then((res) => {
      const { success, data, message } = res.data;
      jwtExpired(message);
      setToken(res.headers && res.headers.authorization);
...
export const updateOpportunityValue = (opportunityId) => {
  return axios
    .put(`${url}/${opportunityId}/calculatedValue`, {}, { headers: headers() })
    .then((res) => {
      const { success, data, message } = res.data;
      jwtExpired(message);
      setToken(res.headers && res.headers.authorization);
...
export const selectLeadSkillResource = (crud, skillId, resId) => {
  return axios
    .patch(
      `${Api}${crud}/${skillId}/allocations/${resId}/mark-as-selected`,
      {},
      { headers: headers() }
    )
    .then((res) => {
