...
export const selectLeadSkillResource = (proId, skillId, resId) => {
  return axios
    .patch(
      url +
        `/${proId}/resources/${skillId}/allocations/${resId}/mark-as-selected`,
      { headers: headers() }
    )
    .then((res) => {
      const { success, message } = res.data;
...
export const getMilestoneExpenses = (crud) => {
  return axios
    .get(`${Api}${crud}`, { headers: headers() })
    .then((res) => {
      const { success, data, message } = res.data;
...
export const addMilestoneExpense = (crud, data, id) => {
  messageAlert.loading({ content: 'Loading...', key: id });
  return axios
    .post(`${Api}${crud}`, data, { headers: headers() })
    .then((res) => {
...
export const getMilestoneExpense = (crud, expenseId) => {
  return axios
    .get(`${Api}${crud}/${expenseId}`, { headers: headers() })
    .then((res) => {
      const { success, data, message } = res.data;
      jwtExpired(message);
      if (success) {
...
export const editMilestoneExpense = (crud, expenseId, data) => {
  messageAlert.loading({ content: 'Loading...', key: expenseId });
  // return { success: false }
  return axios
    .put(`${Api}${crud}/${expenseId}`, data, { headers: headers() })
    .then((res) => {
      const { success, data, message } = res.data;
      jwtExpired(message);
