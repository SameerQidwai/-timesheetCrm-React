...
class Opportunities extends Component {
              ...
              <Menu>
                ...
                {record.type === 1 ? ( //if condition
                  <Menu.Item key="milestone">
                    <Link
                      to={{
                        pathname: `/opportunities/${record.id}/milestones`,
                      }}
                      className="nav-link"
                    >
                      Milestones
          ...
          return (
            ...
            (el.endDate &&
              `${formatDate(el.endDate, true, true)}`
                ...
                .includes(value.toLowerCase())) ||
            (el.bidDate &&
              `${formatDate(el.bidDate, true, true)}`
                .toLowerCase()
                .includes(value.toLowerCase())) ||
            (el.stage &&
              `${O_STAGE[el.stage]}`
  ...
  advancefilter = (value, column, advSearch) => {
    ...
    if (
      ...
      search['bidDate']['value'] ||
      search['entryDate']['value'] ||
      search['panel']['value'].length > 0 ||
      search['stage']['value'].length > 0 ||
      search['status']['value'].length > 0 ||
      search['type']['value'] ||
      search['stateId']['value'].length > 0
    ) {
      const startDate = search['startDate']['value'] ?? [
        '2010-10-19',
        '2010-10-25',
      ];
      const endDate = search['endDate']['value'] ?? [
        '2010-10-19',
        '2010-10-25',
      ];
      const bidDate = search['bidDate']['value'] ?? [
        '2010-10-19',
        '2010-10-25',
      ];
      const entryDate = search['entryDate']['value'] ?? [
        '2010-10-19',
        '2010-10-25',
      ];
      this.setState({
        filterData: data.filter((el) => {
          ...
          return (
            ...
            moment(
              search['bidDate']['value']
                ? formatDate(el.bidDate, true, 'YYYY-MM-DD')
                : '2010-10-20'
          ...
          <Col style={{ marginLeft: 'auto' }}>
            <Row justify="space-between" gutter={'15'}>
              ...
              <Col>
                ...
                >
                  <PlusSquareOutlined /> Add Opportunity
                </Button>
              </Col>
            </Row>
          </Col>
