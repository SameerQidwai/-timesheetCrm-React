...
import { Row, Col, Table, Typography, Modal, Popconfirm, DatePicker, Button, Select } from "antd";
import { CloseCircleOutlined } from "@ant-design/icons"; //Icons
...
class TimeSheet extends Component {
  ...
  Columns = (date, dateString) => {
    ...
    var loopindex = 1
    this.state.columns = column.map((col, index) => { // reCreate the table columns to insert onCell click
      console.log(index)
      if (col.dataIndex === 'project') { // don't change anything for 1st column
        return col; 
      } 
        var day = new Date(date).toLocaleString("en-us", { weekday: "short" }); // to get day Name i.e mon, tue
    ...
    for (let i =loopindex; i<31; i++){ // To pop columns for 31 month and feburary
      this.state.columns.pop() // pop
    }
    this.setState({ // change column state
      columns: this.state.columns
    })
  };

  columnTitle = (day, month, numDate) => { // create column title
    return (
      <>
        <div>{day}</div>
        <div>
  ...
  getRecord = (data, rowIndex, colKey) => { // get record in dialog box for edit
    if (data[colKey]){
      let obj ={
        start: moment(data[colKey].start, 'HH:mm')._isValid ? moment(data[colKey].start, 'HH:mm'): null,
        end: moment(data[colKey].end, 'HH:mm')._isValid ? moment(data[colKey].end, 'HH:mm'): null,
  ...
  Callback = (value) => {
    ...
    this.state.data[this.state['dataIndex']['r']][this.state['dataIndex']['c']] = obj
    ...
    return (
      <>
        <Row justify="space-between">
          ...
          <Col>
            <DatePicker size="large" mode="month" picker="month" format="MMM-YYYY" onChange={this.Columns} defaultValue={moment(new Date())}/>
          </Col>
          <Col>
            <Button size="small" type="primary" onClick={()=>{this.setState({proVisible:true})}}>Add Project</Button>
          </Col>
        </Row>
        <Table
          columns={this.state.columns}
